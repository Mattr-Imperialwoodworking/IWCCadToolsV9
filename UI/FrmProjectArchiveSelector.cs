...
{
    /// <summary>
    /// Archive/legacy two-pane project + dash selector dialog.
    /// Matches FrmProjectSelector UI but loads all project/dash records for old archived drawings.
    ///
    /// Left pane:  filterable project list (from dbo.Proj_Compile)
    ...
    /// </summary>
    public partial class FrmProjectArchiveSelector : IWCBaseForm
    {
        // -----------------------------------------------------------------------
        // Output properties
        // -----------------------------------------------------------------------
        ...
        public FrmProjectArchiveSelector()
        {
            InitializeComponent();
            WireEvents();
        }
        ...
        private async Task LoadProjectsAsync()
        {
            ...
            try
            {
                projects = await Task.Run(() => _repo.GetAllProjects().ToList());
            }
            catch (Exception ex)
            {
                errorMsg = ex.Message;
        ...
        private async Task LoadDashesAsync(int projectId)
        {
            ...
            try
            {
                dashes = await Task.Run(() => _repo.GetAllDashesForProject(projectId).ToList());
            }
            catch (Exception ex)
            {
                errorMsg = ex.Message;
        ...
        private void InitializeComponent()
        {
            ...
            Text           = "Select Archive Project & Drawing Series";
